    ...
    async with aiohttp .ClientSession ()as session :
      try :
        async with session .get (f"http://ip-api.com/json/{ip}")as response :
          ...
          container.add_item(ui.Separator())
          container.add_item(ui.TextDisplay(f'Made bysamaksh-core - [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})'))
          view.add_item(container)
          await ctx.reply(view=view)
      ...
      async with aiohttp .ClientSession ()as session :
        async with session .post (url ,headers =headers ,json =data )as response :
          if response .status ==200 :
            ...
            container.add_item(ui.Separator())
            container.add_item(ui.TextDisplay(f'Powered bysamaksh-core • Requested by [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})'))
            view.add_item(container)
